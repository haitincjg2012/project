package com.ph.shopping.facade.spm.util;
...
import com.ph.shopping.facade.spm.vo.CellBean;
import com.ph.shopping.facade.spm.vo.RowBean;
import com.ph.shopping.facade.spm.vo.SheetBean;
...
public class GenerateXSSFExcel{
    ...
    public void processWorkSheet(SheetBean sheetBean,List<RowBean> rowList,CellStyle cellStyle) throws Exception{
    	if(sheetBean==null){//判断表格数据是否
    		throw new Exception("表格数据不能为空!");
    	}
    	
    	Sheet sheet = null;
    	if(!sheetMap.containsKey(sheetBean.getIndex())){//判断建立几个excel工作目录
    		//初始化sheet
    		sheet = workbook.createSheet(sheetBean.getName() == null ? "sheet" : sheetBean.getName());
    		sheetMap.put(sheetBean.getIndex(), sheet);//新建工作excel
    		merged.clear();//新加sheet时，将前sheet单元格占用表格清空，
    	}else{
    		sheet = sheetMap.get(sheetBean.getIndex());
    	}
    	
    	if(sheetBean.getRowList()!=null&&sheetBean.getRowList().size()>0){//判断写入是否为空
    		if(rowList==null){
    ...
    private void pocessRow(RowBean r, Sheet sheet,int rowIndex,CellStyle cellStyle) throws Exception {
        ...
        if (r.getHeight() != 0) {//高度不等于0的时候设置高度
	...
	private void pocessCell(RowBean r,Row row, Sheet sheet,int rowIndex,CellStyle cellStyle) throws Exception {
		int cellIndex=0;
		String type = "String";
		for (int i = 0; i < r.getCellList().size(); i++) {//循环列
			CellBean  cb = r.getCellList().get(i);//取值  取列出来
            cellIndex = checkMerged(rowIndex,cellIndex);
            Cell cell = row.createCell(cellIndex);//创建每格
            ...
            try {
            	...
            	if (value != null) {
            		...
            		}else if("Date".equalsIgnoreCase(type) && !"".equals(value)){
            			 cell.setCellType(XSSFCell.CELL_TYPE_NUMERIC);
            			 cell.setCellValue(dateToStamp(String.valueOf(value)));
            		} else {
...
//	        			type = vo.getClass().getSimpleName();
    ...
    public static  String convertValue(Object obj){
        ...
        else if(obj instanceof Date) cellValue = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(obj);
        else if(obj instanceof Float || obj instanceof Double)
            cellValue= new BigDecimal(obj.toString()).setScale(2,BigDecimal.ROUND_HALF_UP).toString();
        else cellValue = obj.toString();
        return cellValue;
    }
    ...
    public static String dateToStamp(String s) throws Exception{
          ...
          if(StringUtils.isNotEmpty(s) && s!="null" && !s.equals("0")){
	          long lt = new Long(s);
