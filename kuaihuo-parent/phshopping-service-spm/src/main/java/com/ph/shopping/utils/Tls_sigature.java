...
public class Tls_sigature {//merchan
	/*
	 * public static void main(String[] args) { Tls_sigature aa=new
	 * Tls_sigature(); aa.getUserSign("13241372414mme"); }
	 */
	public static void main(String[] args) {
		String admin="admink";
		Tls_sigature tls_sigature = new Tls_sigature();
		tls_sigature.getUserSign(admin);
	}
	...
	public String getUserSign(String admin) {
		admin = admin.trim();
	...
	/*	InitParams initParams = new InitParams();*/
		...
		try {
			...
			// 私钥
			//String privStr = (String) initParams.getProperties().get("LT_IM_PRIVSTR");
			String pubStr = "-----BEGIN PUBLIC KEY-----\nMFYwEAYHKoZIzj0CAQYFK4EEAAoDQgAEs8tH5lhL+n1BtdGJYO6YTAc/lDMy7SX4\nA7NweVEnFrvWshKhX1hPCRc/IQIvyGgogrdsKhnvapFs2polUht2EA==\n-----END PUBLIC KEY-----";
			// 公钥
			//String pubStr = (String) initParams.getProperties().get("LT_IM_PUBSTR");
	    String privStr = "-----BEGIN PRIVATE KEY-----\nMIGEAgEAMBAGByqGSM49AgEGBSuBBAAKBG0wawIBAQQgX9NukXZNTJrCU6BguIcr\nH+PmXyGIT8cSEcCnWgx0s8mhRANCAASzy0fmWEv6fUG10Ylg7phMBz+UMzLtJfgD\ns3B5UScWu9ayEqFfWE8JFz8hAi/IaCiCt2wqGe9qkWzamiVSG3YQ\n-----END PRIVATE KEY-----";

			//String sdkappid = (String) initParams.getProperties().get("LT_IM_SDKAPPID");
			String sdkappid ="1400039291";

			result = GenTLSSignatureEx(Long.valueOf(sdkappid), admin, privStr);

			// privStr:私钥文件内容
			/*
			 * GenTLSSignatureResult result = GenTLSSignatureEx(1400026262,
			 * "admin", privStr);
			 */
			...
			// 验证签名
			CheckTLSSignatureResult checkResult = CheckTLSSignatureEx(result.urlSig, 1400039291, admin, pubStr);
			if (checkResult.verifyResult == false) {
				System.out.println("CheckTLSSignature failed: " + result.errMessage);
				return "签名生成失败";
			}

			// System.out.println("\n---\ncheck sig ok -- expire time " +
			// checkResult.expireTime + " -- init time " + checkResult.initTime
			// + "\n---\n");
          System.out.println("延签名\n"+result.urlSig);
		} catch (Exception e) {
			e.printStackTrace();
		}
