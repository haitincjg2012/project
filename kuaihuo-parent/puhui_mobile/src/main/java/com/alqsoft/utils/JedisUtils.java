	   ...
	   /* private static final Logger LOGGER = Logger.getLogger(JedisUtils.class);*/
	    ...
	    public JedisUtils() {
	    	/*if (pool == null) {
	            ...
	            //pool = new JedisPool(config, "127.0.0.1", 36379, 100000);
	            //pool = new JedisPool(config, "10.141.111.185", 36379, 100000);
	            InitParamPc initParam = new InitParamPc();
	            String redisHost = initParam.getProperties().get("REDIS_HOST").toString();
	            Integer redisPort = Integer.valueOf(initParam.getProperties().get("REDIS_PORT").toString());
	            pool = new JedisPool(config, redisHost, redisPort, 100000);
	        }*/
		...
		public static JedisUtils getRu(InitParamPc initParam) {
			if (pool == null) {
	    		/*String ip = InitListener.getValue("redis.ip", "192.168.116.207");
	    		int port = Integer.parseInt(InitListener.getValue("redis.port", "5379"));*/
	            JedisPoolConfig config = new JedisPoolConfig();
