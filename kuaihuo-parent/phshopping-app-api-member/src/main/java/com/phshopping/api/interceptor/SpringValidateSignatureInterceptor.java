...
public class SpringValidateSignatureInterceptor extends HandlerInterceptorAdapter{
	...
	private final static String[] publicurls={
			"/api/image/uploadHeadImage",
			"/api/share/shareIndex",
			"/api/share/hideorshow",
			"/api/share/memberShare",
			"/share/memberApp",
			"/share/shoppingApp",
			"/api/personal/score/callbackmsg",
			"/error"
	};
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(requestURI.contains("/api/sms/sendsharecode/")){
			requestURI = "/api/sms/sendsharecode/";
		}
		if(objType == null){
			...
			if(!(objType.toString().equals("I") || objType.toString().equals("A"))){
				...
				String sign = "";
				String md5 = MD5Util.MD5Encode(signData,"utf-8")+"KHHY"+objType.toString();
				byte[] b = null;
			    try {
